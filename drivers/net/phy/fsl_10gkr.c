...
static void xgkr_wq_state_machine(struct work_struct *work)
{
	...
	dev_info(&phydev->mdio.dev, "is training.\n");
	...
	for (i = 0; i < 2;) {
		...
		if (is_link_training_fail(phydev)) {
			...
			s_m->running = false;
			s_m->link_up = true;
			dev_info(&phydev->mdio.dev, "LT training is SUCCEEDED!\n");
			break;
		}
	}
...
serdes_err:
		dev_err(&phydev->mdio.dev, "Unknown SerDes name");
		return -EINVAL;
lane_err:
		dev_err(&phydev->mdio.dev, "Unknown Lane name");
		return -EINVAL;
}
...
mem_err2:
	kfree(xgkr_inst);
mem_err1:
	dev_err(&phydev->mdio.dev, "failed to allocate memory!\n");
	return -ENOMEM;
}
...
static int __init fsl_xgkr_init(void)
{
	return phy_drivers_register(fsl_xgkr_driver,
		ARRAY_SIZE(fsl_xgkr_driver), THIS_MODULE);
}
