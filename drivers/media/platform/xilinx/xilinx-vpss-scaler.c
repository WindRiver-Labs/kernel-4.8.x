...
#define XSCALER_POWER_OFF	(0x1)
#define XSCALER_POWER_ON	(0x0)
 ...
 * The driver needs the following VPSS specific details to be
 * correctly specified in the DT (Bitstream should match device tree entry) :
 *	- initial source and sink media pad formats
 *	- reset gpio line
 *	- number of H-scaler and V-scaler taps
 *	- pixels per clocks the IP uses
 *	- maxiumum lines and maximum pixels
 */
...
static void
calculate_phases(struct xscaler_device *xscaler,
		 u32 width_in, u32 width_out, u32 pixel_rate)
{
	...
	for (x = 0; x < loop_width; x++) {
		...
		for (s = 0; s < nppc; s++) {
			...
			xscaler->H_phases[x] = xscaler->H_phases[x] |
			  (phaseH << (s * 9));
			xscaler->H_phases[x] = xscaler->H_phases[x] |
			  (array_idx << (6 + (s * 9)));
			xscaler->H_phases[x] = xscaler->H_phases[x] |
...
static void
xv_hscaler_load_ext_coeff
	(struct xscaler_device *xscaler, const short *coeff, int ntaps)
{
	int i, j, pad, offset;
	int  nphases = xscaler->max_num_phases;
 ...
 * filter may operate with 10 tap coefficients and pads the remaining
 * coefficients with zeroes.
 *
 * While upscaling the driver will program 6-tap filter coefficients
 * in any N-tap configurations.
 *
 * This selection is adopted by the as it gives optimal
 * video output determined by repeated testing of the IP
 */
static void
xv_hscaler_select_coeff
	(struct xscaler_device *xscaler, u32 width_in, u32 width_out)
{
	const short *coeff;
	u16 hscale_ratio;
	u16 is_scale_down;
	...
	if (is_scale_down) {
		...
		default:
			...
			dev_err(xscaler->xvip.dev,
				"H-Scaler %d Taps Not Supported while downscaling",
				ntaps);
			return;
		}
	} else {
...
static void xv_hscaler_set_coeff(struct xscaler_device *xscaler)
{
	...
	for (i = 0; i < nphases; i++) {
		for (j = 0; j < ntaps / 2; j++) {
			rd_indx = j * 2 + offset;
			val = (xscaler->hscaler_coeff[i][rd_indx + 1] << 16) |
...
static void xv_vscaler_set_coeff(struct xscaler_device *xscaler)
{
	...
	for (i = 0; i < nphases; i++) {
		for (j = 0; j < ntaps / 2; j++) {
			rd_indx = j * 2 + offset;
			val = (xscaler->vscaler_coeff[i][rd_indx + 1] << 16) |
...
static void
xv_vscaler_select_coeff
	(struct xscaler_device *xscaler, u32 height_in, u32 height_out)
{
	const short *coeff;
	u16 vscale_ratio;
	u16 is_scale_down;
	...
	if (is_scale_down) {
		...
		default:
			...
			dev_err(xscaler->xvip.dev,
				"ERR: V-Scaler %d Taps Not Supported while downscaling",
				ntaps);
			return;
		}
	} else {
...
static inline void
xv_procss_reset_block(struct xvip_device *xvip, u32 channel, u32 ip_block)
{
	u32 val;
...
xv_hscaler_set_phases(struct xscaler_device *xscaler)
{
	...
	case XSCALER_PPC_1:
		...
		for (i = 0; i < loop_width; i += 2) {
			lsb = (u32)(xscaler->H_phases[i] &
					(u64)XHSC_MASK_LOW_16BITS);
			msb = (u32)(xscaler->H_phases[i + 1] &
					(u64)XHSC_MASK_LOW_16BITS);
			val = (msb << 16 | lsb);
...
static int xscaler_s_stream(struct v4l2_subdev *subdev, int enable)
{
	...
	if (!enable) {
		...
		gpiod_set_value_cansleep(xscaler->rst_gpio, XSCALER_POWER_OFF);
		usleep_range(100, 150);
		gpiod_set_value_cansleep(xscaler->rst_gpio, XSCALER_POWER_ON);
		usleep_range(100, 150);
...
static int xscaler_probe(struct platform_device *pdev)
{
	...
	dev_dbg(&pdev->dev, "VPSS Scaler Probe Started");
	...
	dev_dbg(xscaler->xvip.dev, "Resetting VPSS Scalar");
